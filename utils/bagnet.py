# based on https://github.com/wielandbrendel/bag-of-local-features-models/blob/master/bagnets/pytorchnet.py
#https://github.com/wielandbrendel/bag-of-local-features-models/blob/master/LICENSE
# add more BagNet classes
...
class BagNet(nn.Module):
    ...
    def __init__(self, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_classes=1000, avg_pool=True, use_fc = True):
        self.inplanes = 64
        ...
        self.avgpool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),nn.Flatten())#nn.AvgPool2d(1, stride=1)
    ...
    def forward(self, x):
        ...
        if self.avg_pool:
            x = self.avgpool(x)#nn.AvgPool2d(x.size()[2], stride=1)(x)
            #x = x.view(x.size(0), -1)
...
def bagnet121(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-121 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[3,3,3,3], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet121')
    return model

def bagnet105(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-105 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[3,3,3,2], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet105')
    return model

def bagnet89(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-89 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[3,3,2,2], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet89')
    return model

def bagnet79(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-79 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[3,2,2,2], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet79')
    return model

def bagnet77(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-77 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[2,2,2,2], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet77')
    return model

def bagnet61(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-61 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[2,2,2,1], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet61')
    return model


def bagnet45(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-45 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[2,2,1,1], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet45')
    return model


def bagnet37(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-37 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[2,1,1,1], **kwargs)
    if pretrained:
        print('no pretrained weight for bagnet37')
    return model
