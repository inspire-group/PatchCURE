	...
	def certify(self,x,y):
		...
		for i,mask in enumerate(mask_list):
			for j in range(i,num_mask):
				...
				prediction_map[:,i,j] = masked_pred.item()
...
class MRPooling(SecurePooling): # for now, for simplicty, I inherit the SecurePooling class. 
	def __init__(self,input_size,mask_size,mask_stride,thres):
		super().__init__(input_size,mask_size,mask_stride)
	...
	def forward(self,x,y):
		...
		y = y.unsqueeze(-1)
		#print(unmasked_pred_correct.shape,(~torch.any(torch.logical_and(one_mask_conf>self.thres,one_mask_pred!=y),dim=1)).shape)
		...
		return correct,certify

class MRPC(MRPooling): # Minority Reports + PatchCURE
	def __init__(self,lrf,input_size,mask_size,mask_stride,thres):
	...
	def certify(self,x,y,thres=None): 
		thres = thres if thres else self.thres
		one_mask_conf,one_mask_pred = self.helper(x)
		torch.any(torch.logical_and(one_mask_conf>self.thres,one_mask_pred!=y))
		return ~torch.any(torch.logical_and(one_mask_conf>self.thres,one_mask_pred!=y),dim=1),torch.all(torch.logical_and(one_mask_conf>self.thres,one_mask_pred==y),dim=1)
